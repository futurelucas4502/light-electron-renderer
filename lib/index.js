...
function rendererAction(filePath, viewData, options, callback) {
    if (retry == false) {
        var html = currentRenderer.currentRenderFunction(fs.readFileSync(process.cwd() + "\\" + filePath, 'utf8'), viewData, options, function (error, html) {
            if (error)
        ...
        retry = false;
        var html = currentRenderer.currentRenderFunction(filePath, viewData, options, function (error, html) {
            if (error)
                throw new Error(error);
            callback(html);
            return;
